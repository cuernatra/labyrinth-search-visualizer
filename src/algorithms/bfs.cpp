...
bool Bfs::isWalkable(NodeState state) const
{
    return state != NodeState::Wall;
}

void Bfs::reset(Grid& grid)
{
...
void Bfs::applyFinalStates(Grid& grid, int foundGoalIndex)
{
...
Bfs::StepResult Bfs::step(Grid& grid, int maxSteps)
{
    if (!running)
